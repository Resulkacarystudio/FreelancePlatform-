...
namespace FreelancePlatform.Controllers
{
    public class KullaniciController : Controller
    {
        ...
        {
            // 🔎 Öncelikle kullanicilar listesinden kontrol et
            var user = kullanicilar.FirstOrDefault(k => k.EmailAdres == email && k.Sifre == sifre);

            // 🔎 Eğer yoksa, Firebase’den kontrol et
            if (user == null)
            {
                var firebaseUser = await FirebaseHelper.GetUserByEmailAsync(email);

                if (firebaseUser != null && firebaseUser.Sifre == sifre)
                {
                    // Firebase’den bulunan kullanıcıyı listeye ekle
                    kullanicilar.Add(firebaseUser);
                    user = firebaseUser;
                }
            }
        ...
        {
            // 🔴 Bellekteki listeye eklemeyi yoruma al
                ...
                var veri = new Dictionary<string, object>
        {
            ...
            {"Rol", yeniKullanici.Rol}
                ...
                FirebaseHelper.AddDataAsync("kullanicilar", yeniKullanici.EmailAdres, veri).Wait();
        ...
        public IActionResult ProfilDuzenle(AppUser guncellenen)
        {
            ...
            if (mevcut != null)
            {
                ...
                // Email ve Rol burada değişmiyor

                // Firebase güncelleme için dictionary oluşturuyoruz
                var veri = new Dictionary<string, object>
        {
            {"KullaniciID", mevcut.KullaniciID},
            {"AdSoyad", mevcut.AdSoyad},
            {"EmailAdres", mevcut.EmailAdres},
            {"Sifre", mevcut.Sifre},
            {"Rol", mevcut.Rol}
        };

                // FirebaseHelper.AddDataAsync async olduğu için .Wait() ile bekliyoruz
                FirebaseHelper.AddDataAsync("kullanicilar", mevcut.EmailAdres, veri).Wait();
            }
